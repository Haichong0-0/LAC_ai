...
class Settings(BaseSettings):
    ...
    embedding_model: str = Field(
        default="BAAI/bge-small-en-v1.5", alias="LAC_EMBEDDING_MODEL"
    )
    ...
    title_list: Path = Field(
        default=PROJECT_ROOT / "data" / "title_list.json", alias="LAC_TITLE_LIST"
    )
    corpus_path: Path = Field(
        default=PROJECT_ROOT / "data" / "corpus.jsonl", alias="LAC_CORPUS_PATH"
    )
